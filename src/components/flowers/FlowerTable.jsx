  ...
  return (
    <div className="table-container">
      <table>
        ...
        <tbody>{flowers.map((flower) => <tr key={flower.id}><td className="id-cell">#{flower.id}</td><td className="name-cell">{flower.nombre}</td><td>{flower.tipo}</td><td><span className="color-tag">{flower.color}</span></td><td>${Number(flower.precio).toFixed(2)}</td><td><span className={flower.stock === 0 ? 'stock-tag empty' : 'stock-tag'}>{flower.stock}</span></td><td><div className="table-actions"><button className="text-button" type="button" onClick={() => onEdit(flower)}>Editar</button><button className="text-button danger" type="button" onClick={() => onDelete(flower)}>Eliminar</button></div></td></tr>)}</tbody>
      </table>
    </div>
  )
}
